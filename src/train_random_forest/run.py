...
def go(args):
    ...
    # Attempt to sanitize X_val for infer_signature
    X_val_for_signature = X_val.copy()

    # Convert all object columns to string for signature inference
    for col in X_val_for_signature.select_dtypes(include=["object"]).columns:
        X_val_for_signature[col] = X_val_for_signature[col].astype(str)

    signature = infer_signature(X_val_for_signature, y_pred)
    ...
    mlflow.sklearn.save_model(
        ...
        input_example=X_val_for_signature.iloc[:5]
    )
